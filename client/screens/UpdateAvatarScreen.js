...
const UpdateAvatarScreen = ({ navigation }) => {
    ...
    const [avatar, setAvatar] = useState(user.avatar ? `http://192.168.172.192:5000/${user.avatar}` : null);
    ...
    const handleUpdateAvatar = async () => {
        ...
        try {
            const res = await axios.put('http://192.168.172.192:5000/api/user/updateAvatar', formData, {
                headers: {
            ...
            setUser(res.data);
            setAvatar(res.data.avatar ? `http://192.168.172.192:5000/${res.data.avatar}?${new Date().getTime()}` : null); // Add cache-busting query parameter

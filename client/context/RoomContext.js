...
export const RoomProvider = ({ children }) => {
    ...
    useEffect(() => {
        const fetchRooms = async () => {
            ...
            if (token) {
                ...
                try {
                    const res = await axios.get('https://chatfun-backend.onrender.com/api/rooms');
                    setRooms(res.data);
                    if (user) {
    ...
    const createRoom = async (name, isPrivate) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.post('https://chatfun-backend.onrender.com/api/rooms/create', { name, isPrivate });
    ...
    const fetchUsersInRoom = async (roomId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.get(`https://chatfun-backend.onrender.com/api/rooms/${roomId}/users`);
                return res.data;
            } catch (err) {
    ...
    const inviteUser = async (roomId, userId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.post('https://chatfun-backend.onrender.com/api/rooms/invite', { roomId, userId });
    ...
    const acceptInvitation = async (roomId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/accept-invitation/${roomId}`);
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
            } catch (err) {
    ...
    const rejectInvitation = async (roomId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/reject-invitation/${roomId}`);
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
            } catch (err) {
    ...
    const changeOwnership = async (roomId, newOwnerId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/change-ownership/${roomId}`, { newOwnerId });
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
    ...
    const setReadOnly = async (roomId, userId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/set-read-only/${roomId}`, { userId });
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
            } catch (err) {
    ...
    const removeReadOnly = async (roomId, userId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/remove-read-only/${roomId}`, { userId });
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
            } catch (err) {
    ...
    const deleteRoom = async (roomId) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                await axios.delete(`https://chatfun-backend.onrender.com/api/rooms/${roomId}`);
    ...
    const changeRoomPrivacy = async (roomId, isPrivate) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/change-privacy/${roomId}`, { isPrivate });
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
                return res.data;
            } catch (err) {
    ...
    const changeRoomColor = async (roomId, backgroundColor) => {
        ...
        if (token) {
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            try {
                const res = await axios.put(`https://chatfun-backend.onrender.com/api/rooms/change-color/${roomId}`, { backgroundColor });
                setRooms(rooms.map(room => room.roomId === roomId ? res.data : room));
                return res.data;
            } catch (err) {

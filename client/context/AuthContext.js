...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState) => {
      if (nextAppState === 'background' || nextAppState === 'inactive') {
        if (user) {
          socket.current.emit('leaveRoom', { userId: user._id, roomId: defaultRoomId });
          socket.current.emit('userOffline', user._id);
        }
      } else if (nextAppState === 'active') {
        if (user) {
          socket.current.emit('joinRoom', { userId: user._id, roomId: defaultRoomId });
  ...
  const initializeSocket = (userId) => {
    ...
    socket.current.on('connect', () => {
      ...
      socket.current.emit('joinRoom', { userId, roomId: defaultRoomId });
    });
  ...
  const fetchUnreadMessages = debounce(async (userId) => {
    try {
      const token = await AsyncStorage.getItem('token');
      axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
  ...
  const login = async (identifier, password) => {
    try {
      ...
      initializeSocket(userRes.data._id);

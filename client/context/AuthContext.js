...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        const loadUser = async () => {
            ...
            if (token) {
                ...
                try {
                    const res = await axios.get('http://192.168.172.192:5000/api/auth/me');
                    ...
                    const roomRes = await axios.get(`http://192.168.172.192:5000/api/rooms/${defaultRoomId}`);
                    if (roomRes.data) {
    ...
    const login = async (identifier, password) => {
        try {
            const res = await axios.post('http://192.168.172.192:5000/api/auth/login', { identifier, password });
            await AsyncStorage.setItem('token', res.data.token);
            console.log('Token stored:', res.data.token); // Log the token
            axios.defaults.headers.common['Authorization'] = `Bearer ${res.data.token}`;
            const userRes = await axios.get('http://192.168.172.192:5000/api/auth/me'); // Fetch user data
    ...
    const register = async (nickname, email, password) => {
        try {
            const res = await axios.post('http://192.168.172.192:5000/api/auth/register', { nickname, email, password });
            await AsyncStorage.setItem('token', res.data.token);
            axios.defaults.headers.common['Authorization'] = `Bearer ${res.data.token}`;
            const userRes = await axios.get('http://192.168.172.192:5000/api/auth/me'); // Fetch user data
            setUser({ ...userRes.data, avatar: userRes.data.avatar || DEFAULT_AVATAR });
    ...
    const sendPrivateMessage = async (recipientId, message) => {
        try {
            ...
            const recipientRes = await axios.get(`http://192.168.172.192:5000/api/user/${recipientId}`);
            ...
            const res = await axios.post('http://192.168.172.192:5000/api/private-messages/send', { recipientId, message });
            return res.data;
        } catch (err) {
    ...
    const markAsRead = async (messageId) => {
        try {
            const token = await AsyncStorage.getItem('token');
            axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            const res = await axios.put(`http://192.168.172.192:5000/api/private-messages/read/${messageId}`);
            return res.data;
        } catch (err) {
    ...
    return (
        <AuthContext.Provider value={{ user, setUser, isAdmin, login, register, logout, addFriend, removeFriend, blockUser, unblockUser, isFriend, isBlocked, sendPrivateMessage, getPrivateMessages, markAsRead, unreadMessages, setUnreadMessages, currentPrivateChatUser, setCurrentPrivateChatUser, setAlertTitle, setAlertMessage, setAlertType, setAlertVisible, setAlertOnConfirm, setAlertOnCancel }}>
            {children}
            <CustomAlert
